...
print(next(package["version"] for package in metadata["packages"] if package["name"] == "meta-rust"))
PY
)
package_root=$(mktemp -d "$output_root/meta-rust-$version-$target-XXXXXXXX")
...
case "$target" in
    *apple*) artifacts=(meta-rust libmeta_ffi.dylib libmeta_ffi.a) ;;
    *) artifacts=(meta-rust libmeta_ffi.so libmeta_ffi.a) ;;
esac
for artifact in "${artifacts[@]}"; do
    cp "$build_root/$artifact" "$package_root/"
done
cp "$workspace/README.md" "$workspace/LICENSE" "$package_root/"
cp -R "$workspace/examples" "$workspace/docs" "$package_root/"
