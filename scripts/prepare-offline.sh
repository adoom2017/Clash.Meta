...
snapshot=$(mktemp -d "$output_root/meta-rust-offline-XXXXXXXX")
...
mkdir -p "$snapshot/scripts" "$snapshot/docs" "$snapshot/.cargo"
...
for doc in rust-progress.md continuation.md handoff.md vless-protocol.md \
    hysteria2-protocol.md offline-build.md core-runtime.md platform-runtime.md \
    compatibility.md release.md; do
    cp "docs/$doc" "$snapshot/docs/"
done
