...
try {
    ...
    $version = ($metadata.packages | Where-Object name -eq 'meta-rust').version
    if (-not $version) { throw 'CLI package version is missing.' }
    $name = "meta-rust-$version-$Target-" + [Guid]::NewGuid().ToString('N').Substring(0, 8)
    ...
    $artifacts = if ($Target.Contains('windows')) {
        @('meta-rust.exe', 'meta_ffi.dll', 'meta_ffi.dll.lib', 'meta_ffi.lib')
    } elseif ($Target.Contains('apple')) {
        @('meta-rust', 'libmeta_ffi.dylib', 'libmeta_ffi.a')
    } else {
        @('meta-rust', 'libmeta_ffi.so', 'libmeta_ffi.a')
    }
    foreach ($artifact in $artifacts) {
        Copy-Item -LiteralPath (Join-Path $buildRoot $artifact) -Destination $packageRoot
    }
    foreach ($entry in @('README.md', 'LICENSE', 'examples', 'docs')) {
