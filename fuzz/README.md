...
For Windows with an existing Ubuntu WSL installation:

```powershell
wsl -d Ubuntu-22.04 -- bash -lc 'export PATH="$HOME/.cargo/bin:$PATH"; export CARGO_TARGET_DIR=/tmp/meta-rust-fuzz-target; cd /mnt/e/app_workspace/Clash.Meta; bash scripts/fuzz.sh'
```
...
without a protocol crash or sanitizer finding. Target counts and the corrected
initial harness assertion are recorded in `docs/vless-protocol.md`.
