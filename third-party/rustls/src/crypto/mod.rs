...
pub trait ActiveKeyExchange: Send + Sync {
    /// Derive an additional peer secret without consuming this exchange.
    /// Opt-in facility for the local session-id authentication hook only.
    fn additional_peer_secret(&self, _peer: &[u8]) -> Result<SharedSecret, Error> {
        Err(Error::General("key exchange cannot derive an additional secret".into()))
    }
