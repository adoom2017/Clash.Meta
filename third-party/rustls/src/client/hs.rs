...
fn emit_client_hello_for_retry(
    ...
    if let Some(customizer) = &config.session_id_customizer {
        if retryreq.is_some() || tls13_session.is_some() || ech_state.is_some() || supported_versions.tls12 {
            return Err(Error::General("custom session id requires a fresh TLS 1.3 handshake without ECH or retry".into()));
        }
        chp_payload.session_id = SessionId::from_custom([0;32]);
        let encoded = HandshakeMessagePayload(HandshakePayload::ClientHello(chp_payload.clone())).get_encoding();
        let kx = key_share.as_ref().ok_or_else(|| Error::General("custom session id requires a key share".into()))?;
        let session_id = SessionId::from_custom(customizer.customize(&encoded, &**kx)?);
        input.session_id = session_id;
        chp_payload.session_id = session_id;
    }
