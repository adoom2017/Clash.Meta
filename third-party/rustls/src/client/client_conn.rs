...
/// Compute a legacy session id before the ClientHello enters the transcript.
pub trait SessionIdCustomizer: Send + Sync + fmt::Debug {
    /// Compute a 32-byte session id from the complete zero-session-id handshake.
    fn customize(
        &self,
        hello: &[u8],
        kx: &dyn crate::crypto::ActiveKeyExchange,
    ) -> Result<[u8; 32], Error>;
}
...
pub struct ClientConfig {
    /// Locally maintained hook for an authenticated legacy session id.
    /// Requires TLS 1.3 without resumption or ECH. Disabled by default.
    pub session_id_customizer: Option<Arc<dyn SessionIdCustomizer>>,
